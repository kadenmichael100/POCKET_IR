...
void setup() {
  ...
  // The buzzer is used for feedback so that the invisible IR pattern can be "heard" by the user
  IrSender.begin(PIN_IR_SEND, ENABLE_LED_FEEDBACK, BUZZER_PIN);
  
  // 4. Initialize OLED over Hardware SPI
  u8g2.begin();
  // Ensures display transfers happen at 8MHz instead of falling back to 100kHz
